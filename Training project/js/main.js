 ...
 * ════════════════════════════════════════════════════════════════
 * SMEMS — main.js
 * الدوال التي ستبقى موجودة بعد ربط الـ Backend
 *
 * ما يبقى هنا:
 *   - Modal open/close  → لعرض تفاصيل الجهاز والطلب (GET من API)
 *   - filterTable       → فلترة جانب العميل (اختياري مع Backend)
 *   - showToast         → إشعارات سريعة بعد عمليات API
 *   - setReportTab / setFormat → UI فقط، لا علاقة بالبيانات
 *
 * ما سيُحذف / يُعدَّل عند ربط الـ Backend:
 *   - showDeviceInfo  → ستستدعي: fetch('/api/devices/{id}') ثم openModal
 *   - showMaintDetail → ستستدعي: fetch('/api/requests/{id}')  ثم openModal
 *   - showEditUser    → ستستدعي: fetch('/api/users/{id}')     ثم openModal
 * ════════════════════════════════════════════════════════════════
 */
...
/* ── Toast ──────────────────────────────────────────────────── */
/*
 * الاستخدام: showToast('تم الحفظ', 'success')
 * النوع: 'success' | 'error' | 'info'
 * يبقى بعد الـ Backend لعرض نتيجة عمليات fetch()
 */
...
/* ── Table Filter ───────────────────────────────────────────── */
/*
 * فلترة جانب العميل — تظل مفيدة حتى مع Backend
 * لتجنب request جديد عند كل بحث (اختياري)
 */
...
/* ── Report Tab / Format Toggle ─────────────────────────────── */
/* UI بحت — يبقى كما هو مع الـ Backend */

// Handle switching between report tabs
...
/*
 * عند الـ Backend: ستستدعي fetch('/api/notifications/{id}/read', {method:'PATCH'})
 * ثم تُحدِّث الـ DOM
 */
function markNotifRead(btn) {
  var item = btn.closest('.notif-item');
  item.classList.remove('unread');
  /* TODO: fetch('/api/notifications/' + item.dataset.id + '/read', { method: 'PATCH' }) */
}
...
function deleteNotif(btn) {
  ...
  /* TODO: fetch('/api/notifications/' + item.dataset.id, { method: 'DELETE' }) */
}
...
function markAllRead() {
  ...
  /* TODO: fetch('/api/notifications/read-all', { method: 'PATCH' }) */
  showToast('All notifications marked as read', 'success');
}

/* ── Device Info Modal ──────────────────────────────────────── */
/*
 * حالياً: يقرأ البيانات من data-* attributes على الزر (Static)
 * عند الـ Backend: استبدل كل المحتوى بـ:
 *   fetch('/api/devices/' + btn.dataset.devId)
 *     .then(r => r.json())
 *     .then(dev => openModal(buildDeviceInfoHTML(dev)))
 */
...
/*
 * عند الـ Backend: fetch('/api/devices/' + btn.dataset.devId + '/history')
 */
...
/*
 * عند الـ Backend: fetch('/api/requests/' + btn.dataset.reqId)
 */
...
/* ── Edit User Modal ────────────────────────────────────────── */
/*
 * عند الـ Backend: fetch('/api/users/' + btn.dataset.userId)
 * ثم PUT /api/users/{id} عند الحفظ
 */
...
/* ── Delete User Confirm ────────────────────────────────────── */
/*
 * عند الـ Backend: fetch('/api/users/' + userId, {method:'DELETE'})
 * ثم احذف الصف من الجدول: document.querySelector('[data-row="'+userId+'"]').remove()
 */
...
/*
 * عند الـ Backend: <form action="/api/requests" method="POST">
 * اجعل الزر submit يُرسل الـ form مباشرة
 * احذف هذه الدالة بالكامل واستبدلها بـ form HTML عادي
 */
function showRequestForm(devId, devName) {
  openModal(
      ...
      '<div class="form-group">' +
        ...
        (devId
          ...
          : '<select class="form-control" name="device_id"><option value="">Select a device...</option></select>'
            /* TODO: عند الـ Backend املأ الـ options من /api/devices */
        ) +
      '</div>' +
      '<div class="form-group">' +
    ...
    '<div class="modal-footer">' +
      '<button class="btn btn-outline" onclick="closeModal()">Cancel</button>' +
      /* TODO: غيِّر onclick ليُرسل fetch POST /api/requests */
...
/*
 * عند الـ Backend: fetch('/api/requests/' + reqId + '/complete', {method:'PATCH', body: JSON.stringify({notes, device_status})})
 */
function showCompleteModal(reqId, devName) {
  openModal(
    ...
    '<div class="modal-footer">' +
      '<button class="btn btn-outline" onclick="closeModal()">Cancel</button>' +
      /* TODO: غيِّر onclick ليُرسل fetch PATCH /api/requests/{reqId}/complete */
...
/* ── Add Device Modal ───────────────────────────────────────── */
/*
 * عند الـ Backend: <form action="/api/devices" method="POST">
 * احذف هذه الدالة بالكامل واستبدلها بـ form HTML مع action
 */
function showAddDeviceModal() {
  ...
  openModal(
    ...
    '<div class="modal-footer">' +
      '<button class="btn btn-outline" onclick="closeModal()">Cancel</button>' +
      /* TODO: غيِّر onclick ليُرسل fetch POST /api/devices */
...
/* ── Helpers ────────────────────────────────────────────────── */
/* يبقيان لأن showDeviceInfo و showMaintHistory يستخدمانهما */
