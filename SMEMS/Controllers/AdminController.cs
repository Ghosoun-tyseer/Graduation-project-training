...
namespace SMEMS.Controllers
{
    ...
    .FirstOrDefault();
     ...
     .Select(a => new UserListItemViewModel
     {
         ...
         CreatedAt = a.CreatedAt
            ...
            var engineers = (await _db.Engineers.ToListAsync())
    .Select(e => new UserListItemViewModel
    {
        ...
        CreatedAt = e.CreatedAt
            ...
            var staff = (await _db.Staff.Include(s => s.Department).ToListAsync())
        ...
        public async Task<IActionResult> DeleteUser(int id, string role)
        {
            switch (role)
            {
                case "Administrator":
                    var a = await _db.Admins.FindAsync(id);
                    if (a != null) _db.Admins.Remove(a);
                    break;
                case "Engineer":
                    var e = await _db.Engineers.FindAsync(id);
                    if (e != null) _db.Engineers.Remove(e);
                    break;
                case "Medical Staff":
                    var s = await _db.Staff.FindAsync(id);
                    if (s != null) _db.Staff.Remove(s);
                    break;
            }
            await _db.SaveChangesAsync();
        ...
        [HttpGet]
        public async Task<IActionResult> Maintenance(string? status, string? risk)
        {
        ...
        private void DrawPieChartPdf(IContainer container, string title, List<ChartSlice> data)
        {
            container.Background(QuestPDF.Helpers.Colors.White)
                ...
                .Padding(12).Column(col =>
                {
                    ...
                    // رسم مبسط: مستطيلات ملونة تمثل النسب
                    ...
                    col.Item().PaddingTop(8).Row(row =>
                    {
                        ...
                        foreach (var slice in data)
                        {
                            if (i++ > 0 && i % 3 == 1)
                            {
                                // سطر جديد كل 3 عناصر - QuestPDF لا يدعم هذا مباشرة
                                // نستخدم Column بدلاً من ذلك
                            }
